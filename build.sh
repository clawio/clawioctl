...
#
# ClawIO Controller build script. Add build variables to compiled binary.
#
# Usage:
#
...
if [[ -z "${current_tag_value}" ]]; then
	...
	GOOS=darwin  GOARCH=amd64 go build -ldflags "${ldflags[*]}" -o ${releases_dir}/"${output_filename}"-${tag_value}-darwin-amd64-${current_date}-${commit_value}
